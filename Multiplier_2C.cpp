...
void Multiplier_2C::GenerateInversionHardware() {
	...
	for (size_t i = 0; i < num_bits_A; ++i) {
		const auto wire = make_shared<Wire>(conv_name + "_Cout");
		...
		if (i != (num_bits_A - 1)) {
			...
			for (size_t level = 0; level < num_and_levels; ++level) {
				ands[level][i]->Connect(PORTS::A, wire);
			}
		}
	...
	for (size_t i = 0; i < num_bits_B; ++i) {
		...
		for (size_t a = 0; a < num_ands_per_level; ++a) {
			ands[i][a]->Connect(PORTS::A, out_wire);
		}
	...
	for (size_t i = 0; i < num_bits_B; ++i) {
		const auto wire = make_shared<Wire>(conv_name + "_Cout");
		input_2C_adders_B[i]->Connect(PORTS::Cout, wire);

		if (i != (num_bits_B - 1)) {
			// Connect the carry out to the B port of the next half adder.
			input_2C_adders_B[i+1]->Connect(PORTS::B, wire);
		} else {
			// The carry out of the last half adder connects to all
			// AND gates at the last level.
			for (size_t a = 0; a < num_ands_per_level; ++a) {
				ands[i][a]->Connect(PORTS::A, wire);
			}
		}

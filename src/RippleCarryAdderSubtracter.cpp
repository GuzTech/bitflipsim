...
void RippleCarryAdderSubtracter::Connect(PORTS port, const wire_t &wire, size_t index) {
	if (index >= num_bits) {
		cout << "[Error] Index " << index << " out of bounds for "
			 << "RippleCarryAdderSubtracter \"" << name << "\".\n";
		exit(1);
	}
	...
	case PORTS::Cout:
		...
		break;
	default:
		cout << "[Error] Trying to connect to undefined port of RippleCarryAdderSubtracter "
			 << "\"" << name << "\".\n";
		exit(1);
	}
}
...
const wire_t RippleCarryAdderSubtracter::GetWire(PORTS port, size_t index) const {
	...
	default:
		cout << "[Error] Trying to retrieve undefined port of RippleCarryAdderSubtracter "
			 << "\"" << name << "\".\n";
		exit(1);
	}
}
